...
float4 main(Output input) : SV_TARGET
{
	...
	return tex.Sample(smp, input.uv) * float4(lerp(ambient, diffuse.rbg, diffuseLight), 1) * lerp(float4(shadowColor.Sample(smp, shadowUV).rgb, 1), 1.0, step(shadowPos.z, shadowDepth.Sample(smp, shadowUV) + 0.001f));
}