...
void Enemy::EnemyAIStateCombat::Approach::Update(float time)
{
	...
	if (m_moveFlag)
	{
		DirectX::XMFLOAT3 moveVector = Crown::Math::VectorNormalize(toPlayerVector);
		moveVector = Crown::Math::VectorScale(moveVector, MOVE_SPEED * time);
		enemy->m_position = Crown::Math::VectorAdd(enemy->m_position, moveVector);
	}

	//	プレイヤーの方を向くよ☆
	enemy->m_rotate.y = Crown::Math::RollNormal(enemy->m_rotate.y);
	...
	//	終了処理だよ☆
	if (Crown::Math::VectorSquareSize(toPlayerVector) < STOP_DISTANCE * STOP_DISTANCE && m_moveFlag)
	{
