...
namespace
{
        ...
        float& exposure) noexcept
    {
        ...
        m_offset = size_t(info - static_cast<const char*>(pSource));
...
HRESULT DirectX::GetMetadataFromHDRMemory(const void* pSource, size_t size, TexMetadata& metadata) noexcept
{
    ...
    size_t m_offset;
    float exposure;
    return DecodeHDRHeader(pSource, size, metadata, m_offset, exposure);
}
...
HRESULT DirectX::GetMetadataFromHDRFile(const wchar_t* szFile, TexMetadata& metadata) noexcept
{
    ...
    size_t m_offset;
    float exposure;
    return DecodeHDRHeader(header, headerLen, metadata, m_offset, exposure);
}
...
HRESULT DirectX::LoadFromHDRMemory(const void* pSource, size_t size, TexMetadata* metadata, ScratchImage& image) noexcept
{
    ...
    float exposure;
    TexMetadata mdata;
    HRESULT hr = DecodeHDRHeader(pSource, size, mdata, m_offset, exposure);
    if (FAILED(hr))
        return hr;
    ...
    // Copy pixels
    auto sourcePtr = static_cast<const uint8_t*>(pSource) + m_offset;
